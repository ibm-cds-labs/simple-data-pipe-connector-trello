...
function oAuthTrelloConnector(){
	...
	var connectorInfo = {
						  ...
						  name: 'Trello (fetch lists and cards)'						// TODO; change connector display name
						};
	...
	this.getTrelloDataSetList = function(boards, pipe, done) {
		...
		trelloOAuthConsumer.getBoards('me', 
			                          function (err, boards) {
	...
	*/
	...
	this.fetchRecords = function( dataSet, pushRecordFn, done, pipeRunStep, pipeRunStats, pipeRunLog, pipe, pipeRunner ){
		...
		trelloOAuthConsumer.getListsOnBoard(dataSet.name, function(err, lists){

        	if(err) {
        		pipeRunLog.error('Error fetching lists for board ' + dataSetNameAndLabelToString(dataSet) + ': ' + util.inspect(err,3));
        		return done('Error fetching lists for board ' + dataSet.label + ': ' + JSON.parse(err.data).error.description);
        	}
     ...
     * @param {Object} pipe - data pipe configuration
     */
	var initializeOAuthConsumer  = function(pipe) {

		if(pipe) {
			// Sample Trello API: https://github.com/norberteder/trello
			trelloOAuthConsumer = new Trello(pipe.oAuth.clientId, 
											 pipe.oAuth.accessToken);
		}
		else {
			trelloOAuthConsumer = null;
		}
	}; // function initializeOAuthConsumer
